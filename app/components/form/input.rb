...
module Components
  module Form
    class Input < Components::Form::Base
      ...
      def error
        @error ||= (errors[attribute].presence unless object.blank? || errors.blank?)
      end

      def render_backdrop
        span class: "absolute inset-0 translate-x-1 translate-y-1 transition-transform" \
                    "group-hover:translate-x-0 group-hover:translate-y-0 #{error ? 'bg-rose-100' : 'bg-mindaro-100'}"
      end
      ...
      def render_field
        raw safe(
          form.public_send(:"#{field}_field",
                           ...
                           class: "w-full relative inline-block border-1 border-black p-4 pe-12 text-sm shadow-sm " \
                                  "#{error ? 'shadow-rose-300' : 'shadow-mindaro-300'}",
